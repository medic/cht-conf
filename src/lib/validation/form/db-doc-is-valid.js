  ...
  execute: async ({ xmlDoc, xformPath }) => {
    ...
    dbDocNodes.forEach(node => {
      if (node.nodeName !== 'group') {
        ...
        const childNodes = Array.from(node.childNodes).filter(n => n.nodeType === 1); // Get element children
        const hasTypeField = childNodes.some(child => child.tagName === 'type' || child.getAttribute('name') === 'type');
        if (!hasTypeField) {
