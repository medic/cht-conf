...
const convertForms = require('../lib/convert-forms/convert-forms');
  ...
  watch: async (api, blockFn, callback = {}, uploadStateOnStart = false) => {
    ...
    fsEventsSubscription = await watcher.subscribe(environment.pathToProject, async (err, events) => {
      ...
      for (const event of events) {
        ...
        if (parsedPath.base === RESOURCE_CONFIG_PATH
          || parsedPath.dir === path.join(environment.pathToProject, RESOURCES_DIR_PATH)
        ) {
          eventQueue.enqueue(async () => {
            await uploadResources();
            return fileName;
